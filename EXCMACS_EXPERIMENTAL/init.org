...
* TODO What to do next
  ...
  - [ ] [[https://github.com/minad/org-modern/issues/129][Org Modern replaces org superstar????]] Why?? I really dont like
...
We will also install straight for better package availability...
...
The ~readme~ for straight states that we should also try and put this in
our early init...
...
** Font
   ...
   #+begin_src emacs-lisp
     ...
     ;; Set attributes for mode-line
     (set-face-attribute 'mode-line nil
                         :background "#reff7700"
                         :foreground "gray20"
                         :box '(:line-width (5 . 5) :color "#ff7700" :style flat-button)
                         :height 1.0
                         :family (cond ((eq system-type 'darwin) "SF Compact Rounded")
                                       ((eq window-system 'x) "Gayathri")))

     ;; Set attributes for mode-line-inactive
     (set-face-attribute 'mode-line-inactive nil
                         :inherit 'mode-line
                         :background "gray"
                         :box '(:line-width (5 . 5) :color "gray"))

     ;; Set attributes for minibuffer-prompt
     (set-face-attribute 'minibuffer-prompt nil
                         :foreground (cond ((and (eq system-type 'darwin) (display-graphic-p)) "dark magenta")
                                           ((and (eq system-type 'darwin) (not (display-graphic-p))) "orange")
                                           ((eq window-system 'x) "orange")
                                           (t "cyan")))
   #+end_src
