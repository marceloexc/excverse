...
#+begin_src emacs-lisp
  (defun my/org-dnd-relative-fix (orig-fn &rest args)
    ;; In Emacs 29 macOS:
    ;; args = (URI ACTION)
    ;; where URI should be a list but may be a string.

    (let* ((uri   (nth 0 args))
           (action (nth 1 args)))

      ;; 1️⃣ Normalize URI to list
      (when (stringp uri)
        (setq uri (list uri)))

      ;; 2️⃣ Relativize file URIs
      (when (and buffer-file-name (listp uri))
        (let ((base-dir (file-name-directory buffer-file-name)))
          (setq uri
                (mapcar
                 (lambda (u)
                   (if (string-prefix-p "file:" u)
                       (let* ((path (substring u 5))
                              (decoded (decode-coding-string
                                        (dnd--unescape-uri path)
                                        (or file-name-coding-system
                                            default-file-name-coding-system)))
                              (abs (expand-file-name decoded)))
                         (if (file-in-directory-p abs base-dir)
                             (concat "file:"
                                     (file-relative-name abs base-dir))
                           u))
                     u))
                 uri))))

      ;; 3️⃣ Call original with corrected args
      (apply orig-fn (list uri action))))

  (advice-add 'org--dnd-multi-local-file-handler
              :around #'my/org-dnd-relative-fix)
#+end_src
* Visual
