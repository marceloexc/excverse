...
#+begin_src emacs-lisp
  (defun my/dnd-get-local-file-name-relative (orig-fn uri &optional must-exist)
    "Like `dnd-get-local-file-name', but return relative paths in Org buffers."
    (if (derived-mode-p 'org-mode)
        (progn
          (message "[dnd] input uri: %S" uri)
          (let* ((f
                  (cond
            ...
            (when f
              (setq f (decode-coding-string (dnd--unescape-uri f) coding)))
            (when (and f must-exist (not (file-readable-p f)))
              (setq f nil))
            (when (and f buffer-file-name)
              (let ((base-dir (file-name-directory buffer-file-name)))
                (when (file-in-directory-p f base-dir)
                  (setq f (file-relative-name f base-dir))
                  (message "[dnd] relativized path: %S" f))))
            (message "[dnd] final result: %S" f)
            f))
      ;; not Org mode → behave exactly like Emacs normally would
      (funcall orig-fn uri must-exist)))

  (advice-add 'dnd-get-local-file-name :around #'my/dnd-get-local-file-name-relative)
#+end_src
* Visual
